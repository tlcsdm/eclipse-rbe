...
public class OverlayImageIcon extends CompositeImageDescriptor {
	...
	protected void drawCompositeImage(int width, int height) {
		// Draw the base image
		CachedImageDataProvider baseProvider = createCachedImageDataProvider(baseImage);
		CachedImageDataProvider overlayProvider = createCachedImageDataProvider(overlayImage);
		drawImage(baseProvider, 0, 0);
		int overlayWidth = overlayProvider.getWidth();
		int overlayHeight = overlayProvider.getHeight();
		switch (location) {
		// Draw on the top left corner
		case TOP_LEFT:
			drawImage(overlayProvider, 0, 0);
			break;

		// Draw on top right corner
		case TOP_RIGHT:
			drawImage(overlayProvider, width - overlayWidth, 0);
			break;

		// Draw on bottom left
		case BOTTOM_LEFT:
			drawImage(overlayProvider, 0, height - overlayHeight);
			break;

		// Draw on bottom right corner
		case BOTTOM_RIGHT:
			drawImage(overlayProvider, width - overlayWidth, height - overlayHeight);
			break;
