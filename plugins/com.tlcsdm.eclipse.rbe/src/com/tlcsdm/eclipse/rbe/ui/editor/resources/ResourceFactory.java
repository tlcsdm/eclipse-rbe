...
import com.tlcsdm.eclipse.rbe.model.workbench.files.PropertiesFileCreator;
...
public abstract class ResourceFactory implements IResourceFactory {
	 ...
	 */
	private Map<Locale, SourceEditor> sourceEditors = new TreeMap<>(new UIUtils.LocaleComparator());
...
//
	 ...
	 * @throws CoreException problem creating factory
	 */
	public static IResourceFactory createFactory(IEditorSite site, IFile file) throws CoreException {
	...
	/**
	 * Creates a resource factory based on given arguments and excluding factories
	 * of the given class.
	 * <p>
	...
	protected static Locale parseBundleName(IResource resource) {
		...
		case 1:
			builder.setLanguage(localeSections.get(0));
			break;
		case 2:
			builder.setLanguage(localeSections.get(0));
			builder.setRegion(localeSections.get(1));
			break;
		case 3:
			builder.setLanguage(localeSections.get(0));
			builder.setRegion(localeSections.get(1));
			builder.setVariant(localeSections.get(2));
			break;
		default:
			break;
		}
	...
	protected SourceEditor createEditor(IEditorSite site, IResource resource, Locale locale) throws PartInitException {
		...
		if (resource != null && resource instanceof IFile) {
			...
			try {
				...
				Class<?> clazz = Class.forName(PROPERTIES_EDITOR_CLASS_NAME);
				textEditor = (TextEditor) clazz.getDeclaredConstructor().newInstance();
			} catch (Exception e) {
			...
			try {
				 ...
				 * ResourceFactory.createEditor(.), and the way we set-up everything, we have
				 * to. Since duplicate calls to init(.) create a memory leak, due to a zombie
				 * ActivationListener registered in AbstractTextEditor, we dispose the first
				 * ActivationListener we just unintentionally created
				 */
